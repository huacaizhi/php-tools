...
{
    /**
     * https://learnku.com/articles/4769/php-identity-card-exact-match-verification
     *
     * 身份证号码的结构：要进行身份证号码的验证，首先要了解身份证号码的编码规则。我国身份证号码多由若干位数字或者数字与字母混合组成。
     * 早期身份证由 15 位数字构成，这主要是在 1980 年以前发放的身份证，后来考虑到千年虫问题，
     * 因为 15 位的身份证号码只能为 1900 年 1 月 1 日到 1999 年 12 月 31 日出生的人编号，所以又增加了 18 位身份证号码编号规则。
     *
     * 1>.18 位身份证号码各位的含义:
     * 1-2 位省、自治区、直辖市代码；
     * 3-4 位地级市、盟、自治州代码；
     ...
     *
     */
    public static $aWeight = array(7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2); //十七位数字本体码权重

    public static $aValidate = array('1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2'); //mod11,对应校验码字符值
     ...
     * @return bool
     */
    public static function isIdCardDate($iY, $iM, $iD)
    {
        $iDate = $iY . '-' . $iM . '-' . $iD;
     ...
     * @return string
     */
    public static function getValidateCode($id)
    {
        $id17 = substr($id, 0, 17);
        $sum = 0;
        $len = strlen($id17);
        for ($i = 0; $i < $len; $i++) {
            $sum += $id17[$i] * self::$aWeight[$i];
        }
        $mode = $sum % 11;
        return self::$aValidate[$mode];
    }
    ...
    {
        if (!self::getIdCardProvince($id)) {
            return false;
        }
        $len = strlen($id);
        if ($len == self::NOW_LENGTH) {
            if (!self::isIdCardDate(substr($id, 6, 4), substr($id, 10, 2), substr($id, 12, 2))) {
                return false;
            }
            ...
            return false;
        } else if ($len == self::OLD_LENGTH) {
            if (!self::isIdCardDate('19' . substr($id, 6, 2), substr($id, 8, 2), substr($id, 10, 2))) {
                return false;
            }
            if (!is_numeric($id)) {
                return false;
     ...
     * @return string|bool
     */
    public static function getIdCardProvince($id)
    {
        if (empty($id)) {
            return false;
        }
        ...
        return isset($area[$index]) ? $area[$index] : false;
    }
}